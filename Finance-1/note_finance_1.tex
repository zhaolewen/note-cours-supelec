...
Les produits d\'eriv\'es comme des options exposent leur vendeur \`a un grand risque de perte, si ce risque n'est pas couvert, la perte peut \^etre consid\'erable. Ainsi, le vendeur de ces produits doit r\'epondre \`a deux questions majeurs:
\begin{enumerate}
...
Supposons le contraire: Notons la valeur de ce produit est $V_0$ en $t=0$, et supposons que le taux de rendement $R'>R$. En $t=0$, on fait la strat\'egie suivante:
\begin{enumerate}
...
Si deux portefeuille ont la m\^eme valeur \`a une maturit\'e $T$, alors ils sont \'egales \`a tout instant avant maturit\'e.
...
On note $Call_t(T,K)$ le prix du call \`a l'instant $t$, de strike $K$, et de maturit\'e $T$. On note aussi $Put_t(T,K)$ le prix du put \`a l'instant $t$ de strike $K$ et de maturit\'e $T$.

Le flux du put \`a maturit\'e est $(K-S_T)_+$, et le flux du call est $(S_T-K)_+$.

On dit que le call est
\begin{equation}
	\left\{\begin{array}{rcl}
		\text{\`a la monnaie} & \text{si} & S_t=K\\
		\text{dans monnaie} & \text{si} & S_t>K\\
		\text{en dehors de la monnaie} & \text{si} & S_t<K
	\end{array}\right.
\end{equation}
...
\end{enumerate}
Ces deux portefeuilles donne la m\^eme valeur au strike, car au $T$
\begin{equation}
	C(T)-P(T) = (S_T-K)_+-(K-S_T)_+=S_T-K
\end{equation}
...
\subsection{Introduction}
Nous avons vu que les contrats \`a terme peuvent \^{e}tre valoris\'es par le principe de non-arbitrage alors que ce principe ne donne que des bornes sur le prix des options. La valorisation de ces produits assez complexes n\'ecessite l'introduction des mod\`eles probabilistes pour d\'ecrire les sc\'enarios possibles du march\'e.
...
On consid\`ere qu'il y a $d+1$ actifs financiers et $2$ dates: $t=0, t=1$. L'al\'ea du mod\`ele est represent\'e par $K$ \'etats dans $\Omega=\{\omega_1,\ldots,\omega_K\}$.
...
\begin{enumerate}
	\item  $V_0^\theta =0$
	\item $P(V_1^\theta\geq 0) = 1$ et $P(V_1^\theta>0)>0$ 
\end{enumerate}
o\`u
\begin{enumerate}
	\item $P(G\geq 0) = 1$
...
\end{equation}
$R=0.25$, $S^0$: Actif sans risque vaut $1$ en $t=0$.
...
pour $S_1^1=4$:
\begin{equation}
q_{2d}=\frac{1+R-d}{u-d}=\frac{1.25-0.5}{2-0.5}=\frac{0.75}{1.5}=\frac{1}{2}
\end{equation}
\begin{equation}
P_1(H_2)=\frac{E[H_2]}{1+R}=\frac{1}{1.25}[\frac{1}{2}\times 7+\frac{1}{2}\times 1]=\frac{16}{5}
\end{equation}
...
\begin{thm}
	Le juste prix $P_0(H_n)$ $(H_n=(S_n^1-K)_+)$ converge quand $n\to\infty$ au prix Black Scholes.
\end{thm}
\begin{equation}
...
\begin{equation}
\exists p, \forall x, |g(x)|\leq c(1+|x|^p)
\end{equation}
...
g(S)=(S-K)_+
\end{equation}
dans le mod\`ele de B\&S est donn\'e par
\begin{equation}
...
Le gamma est d\'efini comme la d\'eriv\'ee du prix au b, de la d\'eriv\'ee du delta.
\begin{equation}
